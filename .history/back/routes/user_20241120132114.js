...
router.get('/news', async (req, res) => {
  try {
    const apiUrl = 'https://api.thenewsapi.com/v1/news/headlines';
    const response = await axios.get(apiUrl, {
      headers: {
        'Authorization': `Bearer ${process.env.NEWS_API_KEY}` // Use your actual API key here
      }
    });
    res.json(response.data);  // Send the fetched news data back to the frontend
